#  ABOUT ME 
Final-year Computer Science and Engineering student at Leading University, Sylhet, Bangladesh, with strong academic training in Python, Machine Learning, Data Analysis, and algorithmic design. Skilled in developing and evaluating ML models, preprocessing complex datasets, and conducting applied research. With a consistent academic record (CGPA 3.56/4.00), I am dedicated to advancing research in Natural Language Processing, Biomedical NLP, and Machine Learning, and committed to contributing to impactful innovations in AI.

# EDUCATION
- B. Sc. in Computer Science and Engineering|Leading University, Sylhet, Bangladesh.|2021 - 2026 (expected)| CGPA (till 4-I): 3.56 (on a scale of 4.00)
- Higher Secondary Certificate |Madan Mohan College, Sylhet, Bangladesh.| 2021 |GPA: 4.67 (out of 5.00)
- Secondary School Certificate |Govt. Agragami Girls’ High School and College, Sylhet, Bangladesh.|2019| GPA: 4.33 (out of 5.00)

# SKILL(S)
** Basic
- MS Word, Excel, PowerPoint Presentation,Spreadsheets, Data Cleaning, Statistical Analysis, Information Management, Web development and design.
  
** Programming Languages and Frameworks
- Python, C, C++, Java, Dart, HTML, Flutter, Data Analysis, Machine Learning, Database design, Algorithm design and implementation, (NLP) techniques
Research writing and academic presentation.

** Others (Hobby)
- Drawing, Photography, Music (Guitar), Creative Thinking, Writing,Team Collaboration, Cultural Exploration.

#  EXTRACURRICULAR ACTIVITIES 
- Developed Pet App and academic websites as part of coursework and applied projects. (Screenshot 2025-11-27 215750.png)
- Completed multiple advanced online courses from Stanford, Harvard, Coursera, and Kaggle in Python, Machine Learning, Data Analysis, and Artificial Intelligence.
- Active participation in Olympiads: Mathematics, Biology, and English, strengthening problem-solving and analytical skills.
- Engaged in independent research and exploration in NLP, Biomedical NLP, and Machine Learning.

# AWARDS AND CERTIFICATES 
- Participated in several Olympiads such as Biology Olympiad (2018), English Olympiad (2019), and Mathematical Olympiad 2021 on 27 March 2021 (Online Regional). 
- Certificate of Achievement Digital Art Competition (UN75 2020 AND BEYOND). (Screenshot_20221111-230313_Gallery.jpg)
- Certificate of Completion - Stanford University's Code in Place 
 
# RESEARCH AND PUBLICATIONS
- MonBarta: Mental Health Detection from Banglish Text – A Biomedical NLP Approach to Emotion Classification in Textual Transliteration . Draft completed; planned submission to international conference and journal within two months.
- Risk Analysis of Cervical Cancer Using Machine Learning and Algorithms. Draft completed; planned submission to research conference and journal within two months.

# LANGUAGE PREFERENCES
- Bangla: Native
- English: Professional (Professional level reading, writing, and speaking)

